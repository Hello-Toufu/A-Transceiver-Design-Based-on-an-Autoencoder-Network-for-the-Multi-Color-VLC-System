...
#         (encoded_constraint)
# aveP_loss = Lambda(lambda x: tf.abs((tf.keras.backend.mean(encoded_constraint, axis=1)) - aveP), name='aveP_loss')\
#         (encoded_constraint)
# aveP_loss = Lambda(lambda x: ((tf.keras.backend.sum(encoded_constraint, axis=1)) - aveP), name='aveP_loss')\
#         (encoded_constraint)
# aveP_loss = Lambda(lambda x: tf.square(tf.reduce_sum(encoded_constraint, axis=1) - aveP), name='aveP_loss')\
#         (encoded_constraint)
aveP_loss = Lambda(lambda x: tf.expand_dims(tf.square(tf.reduce_sum(tf.keras.backend.mean(encoded_constraint, axis=0)) - aveP),axis=0), name='aveP_loss')\
            (encoded_constraint)
...
class MyCallback(Callback):
    ...
    def on_batch_end(self, epoch, logs={}):
        ...
        # results = [1, 1]
        if results[1] > 0:
            K.set_value(self.alpha,  results[0]**2 / (results[0]**2 + results[1]**2) )
        else:
...
print ("sum power:",np.sum(scatter_point, axis=1)  )
