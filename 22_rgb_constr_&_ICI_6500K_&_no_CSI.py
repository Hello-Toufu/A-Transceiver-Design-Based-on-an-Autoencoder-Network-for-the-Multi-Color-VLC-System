...
# 22. CCT + no CSI + multi_xi (not good than constellation design)
############################################################################
...
for i_color in [2]:
    ...
    rec_without_noise = encoded_constraint
    ...
    deco_s1 = autoencoder.layers[-7](dec_input_1)
    ...
    for i_MonteCarlo in range(0, N_MonteCarlo):
        ...
        for n in range(0, len(SNR_range_dB)):
            ...
            test_r1_without_noise = tf.transpose(test_r1_without_noise_1)
            # normalize
