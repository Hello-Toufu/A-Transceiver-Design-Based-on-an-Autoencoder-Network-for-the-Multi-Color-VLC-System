...
# x_mixed = tf.math.divide(tf.reduce_sum(xc_divide_yc * I_mean / c, axis=0),
#                          tf.reduce_sum(yc_reciprocal * I_mean / c, axis=0))
...
#     g11 * tf.square(x_mixed - 0.313) + 2 * g12 * (x_mixed - 0.313) * (y_mixed - 0.337) + g22 * tf.square(
#         y_mixed - 0.337) - epsilon ** 2), axis=0)
...
x_mixed_rgby = tf.math.divide(tf.reduce_sum(xc_divide_yc_rgby*I_mean/c,axis=0),tf.reduce_sum(yc_reciprocal_rgby*I_mean/c,axis=0))
y_mixed_rgby = tf.math.divide(tf.reduce_sum(I_mean/c,axis=0),tf.reduce_sum(yc_reciprocal_rgby*I_mean/c,axis=0))
color_loss = Lambda(lambda x: tf.expand_dims(tf.abs (
                    g11*tf.square(x_mixed_rgby-0.313) + 2 * g12 *(x_mixed_rgby-0.313) * (y_mixed_rgby - 0.337) + g22 * tf.square(y_mixed_rgby - 0.337) - epsilon **2),axis=0)
