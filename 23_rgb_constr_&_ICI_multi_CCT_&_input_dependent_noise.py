...
# 17. consider input-dependent noise (may be wrong)
# 23. not finished
############################################################################
...
for i_color in [0]:
    ...
    SNR_train = 10. ** (15 / 10.)  # coverted 7 db of EbNo
    ...
    #             )
    #         )
    rec_signal_power = tf.reduce_mean(
                    (rec_without_noise - tf.reduce_mean(rec_without_noise)) ** 2
                )
    # rec_without_noise_normalize = rec_without_noise

    # noise_std_train = tf.sqrt(1/SNR_train)
    noise_std_train = tf.sqrt(rec_signal_power/SNR_train)
    rec_with_noise = GaussianNoise( noise_std_train*tf.sqrt(1+psi_2*rec_without_noise)  )\
        (rec_without_noise)
    ...
    for i_MonteCarlo in range(0, N_MonteCarlo):
        ...
        for n in range(0, len(SNR_range_dB)):
            ...
            test_r1_signal_power = tf.reduce_mean(
                    (test_r1_without_noise - tf.reduce_mean(test_r1_without_noise)) ** 2
                )
            noise_std = np.sqrt(test_r1_signal_power / SNR)
            noise = noise_std * nosie_normalize
