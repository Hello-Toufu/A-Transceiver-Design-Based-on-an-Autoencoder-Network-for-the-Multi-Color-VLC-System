...
for i_color = [1  2 3]
    ...
    for i_p_tx=1:length(SNR_dB)
        ...
        for mm = 1:num_loop
            ...
            rec_data_with_noise = rec_data + sqrt(P_noise * rec_data_power)*randn(size(rec_data)) + sqrt(psi_2*rec_data*P_noise* rec_data_power).*randn(size(rec_data));
            %                     rec_data_with_noise = rec_data;
            %             rec_data_with_noise = rec_data_with_noise * rec_data_std;
...
save('RGB_same_h_constellation_design_vs_AE_shot_noise.mat')
